...
import { formatDate, itemName, message, signed, styles, totals, verdict, type Receipt } from './engine';
export default function ReceiptPaper({ receipt }: { receipt: Receipt }) {
  ...
  return <article className="receipt" aria-label="今日生活小票" data-testid="receipt">
    ...
    <div className="receipt-section"><div className="sum-row"><span>初始电量</span><span>100</span></div><div className="sum-row"><span>今日收入</span><span>{signed(total.income)}</span></div><div className="sum-row"><span>今日支出</span><span>{total.expense}</span></div><div className="balance"><div><b>当前电量余额</b><BatteryMedium size={29}/></div><span className="stamp">{verdict(total.balance)}</span><strong data-testid="balance">{total.balance}</strong></div></div>
