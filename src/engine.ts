...
export type Item = { id: string; raw: string; rule: string; quantity: number; unit: number; customName?: string };
...
type Rule = { id: string; re: RegExp; unit: number; names: [string, string, string]; notes: [string, string, string]; count?: RegExp };
const rule = (id: string, re: RegExp, unit: number, names: Rule['names'], notes: Rule['notes'], count?: RegExp): Rule => ({ id, re, unit, names, notes, count });
export const rules: Rule[] = [
  rule('meeting', /会议|开会|开了.*会/, -10, ['会议耐力体验','人类静音观赏会','灵魂暂存会议室'], ['会散了，把自己也领回来。','会议很充实，尤其是日历。','椅子替灵魂打完了卡。'], /([\d一二两三四五六七八九十]+)\s*(?:场|次|个)(?:[^，。；\n]{0,5})会/),
  rule('overtime', /加班|赶工|熬夜工作|通宵|凌晨下班/, -25, ['加班延长套餐','自愿感动公司套装','被工位收养的夜晚'], ['今天的进度条，先停在这里。','公司灯火通明，主要靠你发电。','把月亮从工位上解救出来吧。']),
  ...
  rule('work', /上班|工作|打卡|公司|工位|搬砖/, -10, ['今日工位使用费','出席人生例会','工牌牵着人类散步'], ['人已到岗，快乐可以稍后补签。','出勤已完成，灵魂不参与考核。','工牌今天带着你上了一天班。']),
  rule('exam', /考试|考了|模拟卷|刷题|做题|期中|期末/, -18, ['脑力限时挑战','知识库存盘点','脑细胞集体开卷'], ['分数先放一边，辛苦是真的。','题目没少出，脑电没少花。','脑细胞排队交了卷。']),
  rule('study', /学习|复习|背单词|上课|读书|看书|论文|作业|自习|网课|备考|图书馆/, -10, ['知识慢慢进货','脑容量扩建工程','往脑袋里种一座图书馆'], ['学进去一点，也算往前一点。','知识在增加，电量诚实下降。','有些知识在脑袋里安了家。']),
  rule('missed', /坐过站|错过.*车|赶不上|误车|没赶上|坐反了|下错站/, -18, ['通勤隐藏关卡','城市额外观光税','被地铁随机传送'], ['多走的路，不必再怪自己。','路线很有弹性，主要是被迫的。','地铁今天把你寄往了支线剧情。']),
  rule('traffic', /堵车|挤地铁|早高峰|通勤|地铁|公交|换乘|末班车/, -15, ['通勤大冒险','人类罐头体验券','城市传送带乘坐费'], ['到站了，肩膀也可以放下来了。','人是流动的，车是静止的。','城市把大家装进了同一节罐头。']),
  rule('rain', /淋雨|下雨|暴雨|忘带伞|下雪|冰雹/, -10, ['天气随机加试','免费淋浴体验','云朵忘了拧紧水龙头'], ['如果淋湿了，先照顾好自己。','天气提供的服务，通常不接受退订。','云朵的水龙头今天漏得有点大。']),
  rule('sunset', /晚霞|日落|夕阳|彩虹|风景|星空|日出|月亮/, 20, ['下班晚霞','免费巨幕观影','天空寄来的橘子糖'], ['晚霞这笔，算你赚的。','这么好看的东西，居然没有付费墙。','天空把最后一颗橘子糖留给了你。']),
  rule('early', /早起|被闹钟叫醒|闹钟一响就起/, 8, ['被窝谈判胜利','清晨首班乘客','和被窝和平分手'], ['能早起的人，值得一顿热乎早饭。','闹钟很尽职，你比它还尽职。','被窝签了停战协议，有效期一天。']),
  rule('coffee', /咖啡|美式|拿铁|浓缩|手冲/, 15, ['续命咖啡','液体开机许可证','借来的一小杯太阳'], ['咖啡到账，慢慢喝，慢慢来。','机器需要电，人类需要美式。','杯子里有一颗小太阳在值班。'], /([\d一二两三四五六七八九十]+)\s*杯/),
  rule('tea', /奶茶|果茶|柠檬茶|珍珠/, 15, ['甜度自选的快乐','情绪补丁加珍珠','杯装快乐星球'], ['小小一杯，也是认真哄自己。','今日情绪支持由珍珠提供。','吸管连接上了另一颗甜味星球。'], /([\d一二两三四五六七八九十]+)\s*杯/),
  ...
  rule('meal', /吃饭|吃了|午饭|晚饭|早餐|火锅|烧烤|外卖|好吃|大餐|食堂|饺子|面条|麻辣烫|泡面/, 15, ['好好吃饭补给包','肚子先替我满意了','胃里的小型庆典'], ['好好吃过饭，今天就有一件好事。','生活没想明白，菜单先想明白了。','胃里正在举行一场小型庆典。']),
  rule('cook', /做饭|下厨|烘焙|炒菜|煮汤|烤了|炖了|做了.*饭|一顿饭/, 18, ['人间烟火自制装','厨神短期营业','厨房里的炼金术'], ['这份热乎，是你亲手给自己的。','外卖平台今天少了一单，生活多了点香。','锅铲把普通食材变成了人间。']),
  rule('exercise', /跑步|跑了步|健身|运动|游泳|骑行|瑜伽|打球|散步|走路|晨跑|爬山|徒步|跳操|跳绳|撸铁/, 20, ['身体重启服务','久坐系统更新','把烦恼甩出地球'], ['动一动，给今天换口气。','身体已更新，工位暂不兼容。','有几颗烦恼被汗水带出了轨道。']),
  ...
  rule('rest', /睡觉|午睡|睡了|补觉|睡到|躺平|发呆|休息|放空|懒觉|泡脚|泡澡|冥想|按摩/, 25, ['理直气壮休息券','不产出也能过审','灵魂送去干洗了'], ['什么都不做，也可以是一种照顾。','今日无效忙碌减少，恭喜。','灵魂洗完晒干，明天再穿。']),
  rule('fun', /电影|追剧|游戏|听歌|音乐|演唱会|看剧|综艺|动漫|漫画|小说|脱口秀|[kK][tT][vV]|[kK]歌|首歌/, 18, ['快乐短暂包场','现实暂时未响应','另一个世界的通行证'], ['开心的时间，不算浪费。','现实已最小化，请勿强制恢复。','你去另一个世界透了一口气。'], /([\d一二两三四五六七八九十]+)\s*(?:部|首)/),
...
export const examples = [
  { name: '打工人的一天', text: '开了3场会议；喝了1杯咖啡；挤地铁回家；看到了下班晚霞' },
  { name: '普通但快乐', text: '认真学习了一上午；吃了好吃的火锅；和朋友聊天；摸了邻居家的猫' },
  { name: '今天先躺平', text: '睡到自然醒；做了一顿饭；看了一场电影；什么都没想，发了一会儿呆' },
];
...
export function parseInput(input: string): Item[] {
  ...
  return parts.map((raw, index) => {
    ...
    return { id: `item-${index}`, raw, rule: matched?.id || 'unknown', quantity, unit: matched?.unit || 0 };
  });
}
const styleIndex = (style: Style) => styles.findIndex(s=>s.id===style);
export function itemName(item: Item, style: Style) { return item.customName || rules.find(r=>r.id===item.rule)?.names[styleIndex(style)] || item.raw; }
...
export function message(receipt: Receipt) {
  ...
  const selected=known.find(r=>r.id==='unwell') || [...known].reverse().find(r=>r.unit>0) || known[0];
  return selected?.notes[styleIndex(receipt.style)] || ['这件小事原样收好，今天不必事事都有意义。','这一天没有标准答案，也没必要写汇报。','今天被折成一张纸，交给口袋里的宇宙。'][styleIndex(receipt.style)];
}
export function createReceipt(input: string, style: Style): Receipt { return { id: `LP-${Date.now().toString(36).toUpperCase()}-${crypto.randomUUID().slice(0,6).toUpperCase()}`, date: new Date().toISOString(), input, style, items: parseInput(input) }; }
export function sampleReceipt(): Receipt { return { ...createReceipt(examples[0].text, 'gentle'), example: true }; }
